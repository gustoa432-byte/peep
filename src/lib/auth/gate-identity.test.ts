...
describe("gateIdentityFromHeaders", () => {
  it("verifies the header token end to end and fails closed without it", async () => {
    ...
    process.env.AUTH_PROJECT_ID = "proj-123";
    process.env.AUTH_GATE_ORIGIN = ISSUER;
    try {
      const token = await signToken(key, {
        sub: "user-1",
        email: "viewer@example.com",
      ...
      const withToken = await gateIdentityFromHeaders(
        new Headers({ "x-peep-identity": token }),
        fetchImpl,
      );
      ...
      delete process.env.AUTH_GATE_ORIGIN;
    }
  });

  it("activates on a deployed-shaped request without AUTH_GATE_ORIGIN", async () => {
  ...
  it("verifies a preview-audience token when only AUTH_GATE_ORIGIN is set", async () => {
    const key = await makeKey("k1");
    const { fetchImpl } = staticJwks([key.jwk]);
    delete process.env.AUTH_PROJECT_ID;
    process.env.AUTH_GATE_ORIGIN = ISSUER;
  ...
  it("rejects a preview-audience token when AUTH_PROJECT_ID is set", async () => {
    const key = await makeKey("k1");
    const { fetchImpl } = staticJwks([key.jwk]);
    process.env.AUTH_PROJECT_ID = "proj-123";
    process.env.AUTH_GATE_ORIGIN = ISSUER;
...
describe("gateTokenAudience", () => {
  it("pins app:<id> when AUTH_PROJECT_ID is set, even alongside AUTH_GATE_ORIGIN", () => {
    process.env.AUTH_PROJECT_ID = "proj-123";
    process.env.AUTH_GATE_ORIGIN = ISSUER;
    try {
  ...
  it("pins preview when AUTH_PROJECT_ID is unset", () => {
    delete process.env.AUTH_PROJECT_ID;
    process.env.AUTH_GATE_ORIGIN = ISSUER;
    try {
...
describe("gateIdentityUserInfo", () => {
  it("falls back to a synthetic email and name for sub-only claims", () => {
    assert.deepEqual(
      ...
      {
        id: "User-1",
        email: "user-1@viewer.peep.invalid",
        emailVerified: false,
        name: "Viewer",
      },
    );
