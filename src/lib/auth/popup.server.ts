...
export async function handleAuthPopupRequest(request: Request): Promise<Response> {
  ...
  try {
    const apiRes = await auth.api.signInWithOAuth2({
      ...
      // redirect_uri for the dynamic `localhost` origin.
...
<script type="application/json" id="peep-auth-popup-msg">${payload}</script>
<script>
(function () {
  var el = document.getElementById("peep-auth-popup-msg");
  var msg = { source: "peep-auth-popup", token: null };
