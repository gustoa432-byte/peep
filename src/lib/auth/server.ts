 ...
 * (`AUTH_ISSUER`) via the `genericOAuth` plugin — the broker brokers the
 ...
 *     client** (`./preview`) and derives the preview's `https://localhost`
...
// Kick SQLite bootstrap as soon as the auth server module loads.
void ensureDbReady();

/**
 * Preview secret must outlive module reloads. Process restart clears both.
 */
...
// for any `localhost` callback (see `./preview`).
...
// a dynamic `localhost` host), so we hand Better Auth a dynamic baseURL:
...
// Local `npm run dev` (Vite default). Browsers may send Origin as any of
...
export const SESSION_TOKEN_COOKIE = "__Host-peep-auth.session_token";
...
export const auth = betterAuth({
  ...
  // carries a `Domain` attribute, so a sibling `*.example.test` app cannot "toss" a
  // `Domain=.example.test` session cookie onto this app. `__Host-` requires Secure +
  ...
  advanced: {
    ...
    cookies: {
      ...
      session_data: { name: "__Host-peep-auth.session_data" },
      account_data: { name: "__Host-peep-auth.account_data" },
      dont_remember: { name: "__Host-peep-auth.dont_remember" },
    },
  },
