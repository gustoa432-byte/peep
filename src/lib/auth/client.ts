 ...
 * (`app-env.json`), which selects the dev user (see `use-current-user`);
...
const BEARER_KEY = "peep-auth.bearer-token";
...
/**
 * The sandbox live preview runs this app inside an iframe on a `localhost`
...
function inLivePreview(): boolean {
  return (
    ...
    window.location.hostname.endsWith(".invalid")
  );
}

/** Message the popup posts back to the opener once sign-in completes. */
type PopupMessage = { source: "peep-auth-popup"; token: string | null; error?: string };

/**
 * Start sign-in with one upstream provider (`providerId` from `AUTH_PROVIDERS`),
 * federating through the auth broker.
 *
 * - **Live preview** (`localhost` iframe): opens a POPUP to
...
function openSignInPopup(providerId: string): Window | null {
  ...
  const name = `peep-signin-${Date.now()}`;
...
function waitForPopupToken(popup: Window): Promise<string | null> {
  return new Promise((resolve) => {
    ...
    const onMessage = (event: MessageEvent) => {
      ...
      if (!data || data.source !== "peep-auth-popup") return;
